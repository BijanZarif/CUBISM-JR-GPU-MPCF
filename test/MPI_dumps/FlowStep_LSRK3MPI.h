...
class FlowStep_LSRK3MPI : public FlowStep_LSRK3
{
	...
	template<typename Kflow, typename Kdiff, typename Kupdate, typename Kupdate_state>
	struct LSRKstepMPI
	{
		LSRKstepMPI(TGrid& grid, Real dtinvh, const Real current_time)
		{
            ...
            //printf("ready to do step 0\n");
            timings.push_back(step(grid, vInfo, 0      , 1./4, dtinvh, current_time));//printf("step 1 done\n");
            timings.push_back(step(grid, vInfo, -17./32, 8./9, dtinvh, current_time));//printf("step 2 done\n");
            timings.push_back(step(grid, vInfo, -32./27, 3./4, dtinvh, current_time));//printf("step 3 done\n");
		...
		pair<double, double> step(TGrid& grid, vector<BlockInfo>& vInfo, Real a, Real b, Real dtinvh, const Real current_time)
		{
			...
			if(LSRK3data::nu1>0)
            {
			    LSRK3data::Diffusion<Kdiff, Lab> diffusion(dtinvh, LSRK3data::nu1, LSRK3data::nu1);
                ...
                {
                    vector<BlockInfo> avail = synch.avail(LSRK3MPIdata::GSYNCH);
                    _process< LabMPI >(avail, diffusion, (TGrid&)grid, current_time, record);
                }
            }
#ifdef _USE_HPM_
            ...
            {
                LSRK3data::Update_State<Kupdate_state> update_state(&vInfo.front(), LSRK3data::min_G, LSRK3data::min_P);
            ...
            {
                LSRK3data::Update_Sponge update_sponge(LSRK3data::p_ref, &vInfo.front());
...
public:
	...
	Real operator()(const Real max_dt)
	{
		...
		if (MPI::COMM_WORLD.Get_rank()==0)
            if(isnan(maxSOS))
		    {
                cout << "Finishing...sos too high" << endl;
                return 0;
		    }
#ifdef _USE_HPM_
		...
		if (LSRK3data::nu1>0)
            dt = min(dt, (double)(h*h/(12*LSRK3data::nu1)));
		...
		if (maxSOS>1e6 && MPI::COMM_WORLD.Get_rank()==0)
	    {
			cout << "Speed of sound is too high. Is it realistic?" << endl;
			MPI::COMM_WORLD.Abort(1);
	    }
        
		if (dt<std::numeric_limits<double>::epsilon() * 1e1 && MPI::COMM_WORLD.Get_rank()==0)
	    {
		...
		if (parser("-kernels").asString("cpp")=="cpp")
		    if (parser("-hllc").asInt()==1)
                LSRKstepMPI<Convection_CPP_HLLC, Diffusion_CPP, Update_CPP, Update_State_CPP>(grid, dt/h, current_time);
		    else
                LSRKstepMPI<Convection_CPP, Diffusion_CPP, Update_CPP, Update_State_CPP>(grid, dt/h, current_time);
#if defined(_QPX_) || defined(_QPXEMU_)
            else if (parser("-kernels").asString("cpp")=="qpx")
                if(parser("-hllc").asInt()==1)
                    LSRKstepMPI<Convection_QPX_HLLC, Diffusion_CPP, Update_QPX, Update_State_QPX>(grid, dt/h, current_time);
                else
                    LSRKstepMPI<Convection_QPX, Diffusion_CPP, Update_QPX, Update_State_QPX>(grid, dt/h, current_time);
#endif
                else
                {
