...
public:
    ...
    void dumpStatistics(G& grid, const int step_id, const Real t, const Real dt)
    {
        ...
        double rInt=0., uInt=0., vInt=0., wInt=0., eInt=0., vol=0., vol2=0., vol3=0., vol4=0., vol5=0., ke=0., r2Int=0., mach_max=-HUGE_VAL, p_max=-HUGE_VAL, kin_ke=0., rho_min=HUGE_VAL;
        double g_rInt=0., g_uInt=0., g_vInt=0., g_wInt=0., g_eInt=0., g_vol=0., g_vol2=0., g_vol3=0., g_vol4=0., g_vol5=0., g_ke=0., g_r2Int=0., g_mach_max=-HUGE_VAL, g_p_max=-HUGE_VAL, g_kin_ke=0., g_rho_min=HUGE_VAL;
        ...
        for(int i=0; i<(int)g_vInfo.size(); i++)
        {
            ...
            const bool bBoundary =
                (info.index[0]==0) || (info.index[0]==grid.getBlocksPerDimension(0)-1) ||
                (info.index[1]==0) || (info.index[1]==grid.getBlocksPerDimension(1)-1) ||
                (info.index[2]==0) || (info.index[2]==grid.getBlocksPerDimension(2)-1);
            
            //just do the analysis and integrals for non-sponge blocks
            if(parser("-sponge").asInt()==1 && bBoundary)
                continue;
            ...
            for(int iz=0; iz<FluidBlock::sizeZ; iz++)
                for(int iy=0; iy<FluidBlock::sizeY; iy++)
                    for(int ix=0; ix<FluidBlock::sizeX; ix++)
                    {
                        if(b(ix, iy, iz).rho<0)
                        {
                            printf("negative density...exiting at step id %d\n", step_id);
                            MPI::Finalize();
                        }
                        
#ifndef NDEBUG
                        Real test[7];
                        StreamerGridPoint sgp;
                        sgp.operate(b(ix, iy, iz), test);
                        for(int comp=0;comp<7;comp++)
                            if(isnan(test[comp]))
                            {
                                Real cur_pos[3];
                                info.pos(cur_pos,ix,iy,iz);
                                printf("nan channel %d at block %d at (%d,%d,%d) and rank %d, position is %e %e %e\n", comp, i, ix, iy, iz, MPI::COMM_WORLD.Get_rank(), cur_pos[0], cur_pos[1], cur_pos[2]);
                                
                                for(int idz=0; idz<4;idz++)
                                {
                                    for(int idy=-3; idy<4;idy++)
                                    {
                                        for(int idx=-3; idx<4;idx++)
                                            printf("%d %d %d: %e ", idx, idy ,idz, b(ix+idx, iy+idy, iz+idz).energy);
                        ...
                        rho_min = min(rho_min, (double)b(ix, iy, iz).rho);
                        ...
                        vol  += b(ix,iy,iz).G > (0.1*min(1/(Simulation_Environment::GAMMA1-1),1/(Simulation_Environment::GAMMA2-1))+0.9*max(1/(Simulation_Environment::GAMMA1-1),1/(Simulation_Environment::GAMMA2-1)))? 1:0;
                        vol2 += b(ix, iy, iz).G > 0.5*(1/(Simulation_Environment::GAMMA1-1)+1/(Simulation_Environment::GAMMA2-1))? 1:0;
                        vol3 += b(ix,iy,iz).G > (0.01*min(1/(Simulation_Environment::GAMMA1-1),1/(Simulation_Environment::GAMMA2-1))+0.99*max(1/(Simulation_Environment::GAMMA1-1),1/(Simulation_Environment::GAMMA2-1)))? 1:0;
                        vol4 += b(ix,iy,iz).G > (0.03*min(1/(Simulation_Environment::GAMMA1-1),1/(Simulation_Environment::GAMMA2-1))+0.97*max(1/(Simulation_Environment::GAMMA1-1),1/(Simulation_Environment::GAMMA2-1)))? 1:0;
                        vol5 += b(ix,iy,iz).G > (0.07*min(1/(Simulation_Environment::GAMMA1-1),1/(Simulation_Environment::GAMMA2-1))+0.93*max(1/(Simulation_Environment::GAMMA1-1),1/(Simulation_Environment::GAMMA2-1)))? 1:0;
                        ...
                        kin_ke   += 1.0/pow(b(ix, iy, iz).rho,2.0) * (b(ix, iy, iz).u*b(ix, iy, iz).u+b(ix, iy, iz).v*b(ix, iy, iz).v+b(ix, iy, iz).w*b(ix, iy, iz).w);
        ...
        MPI::COMM_WORLD.Reduce(&vol2, &g_vol2, 1, MPI::DOUBLE, MPI::SUM, 0);
        MPI::COMM_WORLD.Reduce(&vol3, &g_vol3, 1, MPI::DOUBLE, MPI::SUM, 0);
        MPI::COMM_WORLD.Reduce(&vol4, &g_vol4, 1, MPI::DOUBLE, MPI::SUM, 0);
        MPI::COMM_WORLD.Reduce(&vol5, &g_vol5, 1, MPI::DOUBLE, MPI::SUM, 0);
        ...
        MPI::COMM_WORLD.Reduce(&kin_ke, &g_kin_ke, 1, MPI::DOUBLE, MPI::SUM, 0);
        MPI::COMM_WORLD.Reduce(&rho_min, &g_rho_min, 1, MPI::DOUBLE, MPI::MIN, 0);
        
        if (MPI::COMM_WORLD.Get_rank()==0)
        {
            FILE * f = fopen("integrals.dat", "a");
            fprintf(f, "%d %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e\n", step_id, t, dt, g_rInt*h3, g_uInt*h3,
                    g_vInt*h3, g_wInt*h3, g_eInt*h3, g_vol*h3, g_ke*h3, g_r2Int*h3, g_mach_max, g_p_max, pow(0.75*g_vol*h3/M_PI,1./3.), g_wall_p_max, g_kin_ke*h3/pow(u0_rms,2), g_vol2*h3, g_vol3*h3, g_vol4*h3, g_vol5*h3, g_rho_min);
            fclose(f);
        }
