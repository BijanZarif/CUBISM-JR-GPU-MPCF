...
public:
	...
	virtual void write(const int ix, const int iy, const int iz, WaveletTexture3D& texture)
	{
		...
		const size_t retval = fwrite(ptr, sizeof(unsigned char), nbytes, myfile);

		printf("My nbytes is %d, retval is %d\n", nbytes, retval);
		abort();
	}
...
public:
	...
	void write(const int ix, const int iy, const int iz, WaveletTexture3D& texture)
	{
		...
		const size_t mylutoffset = sizeof(entry) * (ix + xtextures * (iy + ytextures * iz));
		//MPI::Status st_first;
		myfile.Write_at(lutfile_start + mylutoffset, &entry, sizeof(entry), MPI::CHAR);//, st_first);
		//printf("finished writing lut %d bytes\n", st_first.Get_count(MPI::CHAR));
		
		//write data
		assert(ptr != NULL);
		assert(nbytes != 0);
		//MPI::Status st_second;
		myfile.Write_at(myoffset, ptr, nbytes, MPI::CHAR);//, st_second);
		//printf("finished writing data %d bytes\n", st_second.Get_count(MPI::CHAR));
	}
};
